...
- reproducing the behavior directly in `devkit` would be less clear
...
- `build_dir` always
- `source_dir` when `devkit` must configure/build before `ctest`
...
- hiding the real backend command from `devkit`
...
1. Confirm every configured backend exists in `devkit`.
2. Confirm each path is relative to the target repository root where `devkit.yml` will live.
3. Validate with `devkit validate`.
4. Inspect with `devkit inspect`.
