...
def build_image(repo_root: Path, image_name: str) -> None:
    """Build the Docker image for the Arrow example."""
    ...
    subprocess.run(
        [
            ...
            "build",
            "-f",
            str(repo_root / "examples" / "arrow" / "Dockerfile"),
            "-t",
            image_name,
            str(repo_root),
        ],
...
def run_container(image_name: str, command: list[str]) -> int:
    """Run the Arrow example container with the requested command."""
...
def main(argv: list[str]) -> int:
    """Build the Arrow example image and run a command inside it."""

    require_docker()
    repo_root = Path(__file__).resolve().parents[2]
    image_name = os.environ.get("FOGA_EXAMPLE_IMAGE", "foga-example-arrow")
